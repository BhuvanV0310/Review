...
// Demo accounts removed - users must login with admin-provided credentials
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate admin key if admin login is selected
    if (isAdminLogin && form.adminKey !== "Admin123") {
    ...
    try {
      const res = await loginUser(form.email, form.password, isAdminLogin ? form.adminKey : undefined);
      if (res.success) {
      ...
      <Card className="w-full max-w-md relative z-10 bg-white/95 backdrop-blur-sm border-0 shadow-2xl shadow-blue-500/10 transition-all duration-500 hover:shadow-3xl hover:shadow-blue-500/20 animate-slide-up">
        ...
        <CardContent className="space-y-6 px-8 pb-8">
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {/* Admin Key Input - Only show for admin login */}
            {isAdminLogin && (
              <div className="relative group">
                <Input
                  name="adminKey"
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button
              ...
              onClick={() => {
                ...
                setForm({ email: "user@example.com", password: "user123", adminKey: "" });
                setError(null);
              }}
            >
            ...
            <Button
              ...
              onClick={() => {
                ...
                setForm({ email: "test-admin@example.com", password: "TestAdmin123!", adminKey: "Admin123" });
                setError(null);
              }}
            >
              <span className="text-lg">⚡</span>
          ...
          <div className="text-center pt-4 border-t border-slate-100">
            <p className="text-slate-600 text-sm">
              {isAdminLogin 
                ? "Admin accounts require the admin key for registration." 
                : "User accounts are created by administrators. Contact your admin for login credentials."
              }
            </p>
          </div>
        </CardContent>
